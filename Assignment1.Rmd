...
```{r}
soldp_quant <- quantile(data$Med_Sale_Price, na.rm = TRUE)
listp_quant <- quantile(data$Median_List_Price_3, na.rm = TRUE)
inventory_quant <- quantile(data$ForSale_Inventory, na.rm = TRUE)
```
...
```{r}
soldp_st_dev <- sd(data$Med_, na.rm = TRUE)
listp_st_dev <- sd(data$pop_dens, na.rm = TRUE)
inventory_st_dev <- sd(data$covid_rate, na.rm = TRUE)
```
