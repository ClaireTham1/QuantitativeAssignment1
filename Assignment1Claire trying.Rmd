...
cont_summary <- tibble(
  ...
  Median = c(soldp_quartiles[3],
             listp_quartiles[3],
             inventory_quartiles[3]),
  `Interquartile range` = c(soldp_quartiles[4] - soldp_quartiles[2],
                            listp_quartiles[4] - listp_quartiles[2],
                            inventory_quartiles[4] - inventory_quartiles[2]),
...
```{r}
pct_highincome <- t.test(data$MedIncome_Quality == "High Income")
pct_lowincome <-  t.test(data$MedIncome_Quality == "Low Income")
```

```{r}
pct_highlyburdened <- t.test(data$MortBurd_Quality == "Highly Burdened")
pct_lessburdened <-  t.test(data$MortBurd_Quality == "Less Burdened")
```

```{r}
cat_summary_party <- tibble(`Income` = 
                              c("High Income",
                                "Low Income"),
                            `Sample proportion` = 
